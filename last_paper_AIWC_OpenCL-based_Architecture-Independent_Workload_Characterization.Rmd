...
Dallas, Texas, November 12, 2018

LLVM is increasingly used in work targeted at high-performance computing. Research in, and implementation of, program analysis, compilation, execution, and profiling has clearly benefited from the availability of a high-quality, freely-available infrastructure on which to build.
...
#pdf(file='../figure/tmp.pdf')
plot_kiviat(y,nv,colour="black",colour_spokes=TRUE,all_sizes=TRUE)
z2 <- recordPlot()
#plot_kiviat(y,nv,colour="black",colour_spokes=TRUE,all_sizes=TRUE)
#z2 <- recordPlot()
#dev.off()
#z2 <- image_read('../figure/tmp.pdf')
#z2 <- ggdraw() + draw_image(image_trim(z2))
...
We present metrics for `r length(unique(featdata.all$application))` different application codes -- which includes `r length(unique(featdata.all$kernel))` kernels in total.
...
```{r eval=TRUE, dev='pdf', cache=TRUE, echo=FALSE, include=FALSE, draw_stacked_plots, fig.cap = "\\label{fig:stacked_plots} Selected AIWC metrics from each category over all kernels and 4 problem sizes.", fig.width=8.3, fig.height=11.7}

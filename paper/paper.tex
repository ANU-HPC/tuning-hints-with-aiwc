...
\begin{document}
	...
	%It is possible for an engineer to write highly thread-divergent logical analysis code aimed at traditional CPUs; parallelizable data crunching kernels aimed at GPUs; and code to satisfy specialized computational requirements aimed at FPGAs -- all in a single code base using OpenCL.
	%The language provides support for individual \emph{kernels}, portions of parallelizable codes that make up a HPC code base, to be executed independently on diverse hardware targets such as traditional CPUs, GPUs and FPGAs \cite{khronosopenclworkinggroup2011opencl}.
	...
	On the other hand, GPUs rely on hardware multi-threading to hide memory latency, and their architectures favour ALU capability over sophisticated logic to manage a cache hierarchy and out-of-order execution. As a result, the performance of kernels on CPUs and GPUs alike is strongly dependent on memory access patterns intrinsic to the code. % We discuss the memory model design and the manufacturer recommended optimizations for NVIDIA GPUs in later sections..
	...
	In section~\ref{guidelines} we summarize our findings and present guidelines for interpreting the new parallel spatial locality metric for OpenCL codes.
	...
	\begin{figure*}[h!]
		...
		\subfloat{
			...
			\addplot [mark=*, color=brown] coordinates {%[mark=triangle*] coordinates {
				(simple,46.920) %118.94
				(coalA,8.783) %635.39
				(coalAB,9.238) %	604.10
				(coalABT,5.307) %404.45356548
				%(alignABT,5.272)
			};
			\addplot [mark=star] coordinates {%[mark=square] coordinates {
				(simple,130.682) %131.431
				(coalA,28.602) %600.506
				(coalAB,27.939) %614.75
				(coalABT,16.179) %1061.60
				%(alignABT,16.071)
			};	
			\end{axis}
			\end{tikzpicture} 
	...
	We see an improvement to 600 GFLOP/s in the effective performance for order 2048 matrix multiplication.
	...
	Performance is further improved to 614 GLOP/s after this optimization.
	...
	We achieve 1061 GFLOP/s after applying all the recommended GPU memory optimizations to the simple matrix multiply kernel.
	...
	This is directly correlated to cache reuse rates, critical to code performance on CPUs \cite{inteloptimisation}. %Intel optimization manual.
	...
	During the simulation of the target code, CuMAPz records all memory accesses in various buffers to simulate the global and shared memories on NVIDIA GPUs.
	...
	%TODO: Are we making our work sound too much like CuMAPz? This is of course not true and we should fix Reviewer 2's confusion as to what we are doing differently.
	...
	AIWC also records the global memory address entropy (GMAE), a positive real number corresponding to the randomness of the memory access distribution of a program. To measure locality of memory accesses, AIWC collects the local memory address entropy (LMAE) of memory addresses accessed after dropping $n$ least significant bits of all memory addresses accessed by the program. To calculate this, AIWC collects a frequency distribution of all non-register memory accesses by all threads in the target kernel. Using the collected frequency distribution, AIWC calculates 10 separate local memory address entropy (LMAE) values according to increasing the number of least significant bits (LSB) skipped using
	...
	\begin{table*}[h!]
		...
		\begin{tabular}[t]{lrrrrr}
			\toprule
			& simple & coalescedA & coalescedAB & coalescedABT \\ \midrule %& alignedABT \\ \midrule 
			Total memory footprint        & 196608 & 196608     & 196608      & 196608      \\ %\hline
			90\% Memory Footprint         & 118196 & 56176      & 489         & 489         \\ %\hline
			Global MAE                    & 17.02  & 13.18      & 9.78        & 9.78        \\ %\hline
			LMAE \#bits=3                 & 16.02  & 12.18      & 8.78        & 8.78        \\ %\hline
			LMAE \#bits=10                & 9.02   & 5.18       & 1.78        & 1.78        \\ %\hline
	...
	\textit{Memory Footprint}: Similar to trends in global and local MAE, we find that the ratio of 90\% memory footprint to total memory footprint decreases from $60.12\%$ for \texttt{simple} to $0.25\%$ (\texttt{coalescedAB}). Increased utilization of local memory in the optimized kernels means that the local memory buffers make up a greater proportion of total memory accesses in the program. As the local memory buffers are small and reused within a work-group, the memory footprint of the local memory buffers is also smaller. 
	...
	Further, we find that all current AIWC metrics do not measure any sizeable difference between \texttt{coalescedAB} and \texttt{coalescedABT} codes.%and \texttt{alignedABT} codes.
	...
	\section{A Parallel Spatial Locality Metric} \label{discussion}
		...
		\end{axis}
		\end{tikzpicture}
		\caption{Parallel spatial locality metric for matrix multiply kernels for 256 $\times$ 256 matrix multiplication}
	...
	Different ways of aggregating program measurements lead to different features of program execution being emphasized in the final metrics.
...
%	With $E_{n,t}$ the n-bits-dropped entropy at timestamp $t$.
	...
	\begin{figure*}[h!]
		...
		\end{axis}
		\end{tikzpicture}
		\caption{Normalized parallel spatial locality metric for selected OpenDwarfs benchmark kernels}
	...
	This leads to poor parallel spatial locality. Thus, we see a slow drop-off in parallel spatial locality for \texttt{BFS kernel 1}.
	...
	\section{Guidelines: Using the Parallel Spatial Locality Metric} \label{guidelines}
	
	 The recorded parallel spatial locality in GEM starts at a low value of 0.08604 and approaches the theoretical lower limit of 0 (2.18e-4 at 10 bits dropped). This indicates that almost all memory accesses made by the kernel are perfectly synchronized between OpenCL threads. From a memory performance standpoint, this is the best possible scenario as single memory accesses service all the active synchronized threads on the GPU.
	 
	 Codes with a mix of favourable and unfavourable access patterns (typically involving two-dimensional matrices such as in LUD and SRAD) will have a steep drop-off in PSL to about half of the starting PSL metric as $n$ increases, and the curve flattens out at that point.
	 
	 In the extreme case, when parallel memory accesses are very distant as for Needleman-Wunsch, we observe a flat parallel spatial locality graph. 
	 The worst possible memory access pattern occurs when all threads in a work-group access distant memory locations. This results in a flat PSL metric of 1. 
	 The PSL metric for Needleman-Wunsch is a flat line at 0.53. This indicates that parallel memory accesses are entirely disparate. 
	 ...
	 Codes with irregular or data-driven memory access patterns like CSR and BFS in the OpenDwarfs Suite have a slow drop-off in the PSL metric at low $n$ values. This is reflective of the mostly disparate parallel memory access pattern in these codes. The increasing steepness of the drop-off at higher $n$ in codes with data-driven memory accesses is reflective of the size of buffers being operated upon as the data structures used during computation cause localization of memory accesses.
	 
	%TODO: Compile list of guidelines
	
	\begin{comment}
	\section{Predicting OpenCL Kernel Performance using AIWC}
	
	\begin{comment}
	- AIWC performance prediction paper
	- Important for workload schedulers and compiler writers.
	- Used in this paper to demonstrate PSL is inherently linked to performance-prediction.
	
	-- Methodology:
		- Leave-one-out predictive accuracy
		- CLgen?
			- intro
			- use to generate kernels for performance prediction	
	
	In~\cite{beauperformanceprediction} we show that AIWC features can be used to build performance prediction models for the runtime of OpenCL codes on diverse heterogeneous platforms.
	This is useful for workload schedulers at supercomputer facilities and compiler engineers who must predict performance of codes on architectures without gathering performance data directly.
	In this paper, we test whether the implemented parallel spatial locality metric is linked to code performance by building a predictive model
	
	- Used random forest model (ranger).
		
	- 
	\end{comment}
	\section{Conclusions and Future Work} \label{future work}
	
	To the best of our knowledge, this work is the first to propose the use of architecture-independent metrics of parallel memory access to characterize hardware-specific optimization. 

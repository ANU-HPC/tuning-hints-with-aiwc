...
\begin{document}
	...
	In section~\ref{guidelines} we summarize our findings and present guidelines for interpreting the new parallel spatial metric for OpenCL codes.
	...
	Performance is further improved to 614 GLOP/s after this optimisation.
	...
	In the OpenCL programming model, threads within a work-group execute the same code and share access to \textit{local memory}.
...
%	With $E_{n,t}$ the n-bits-dropped entropy at timestamp $t$.
	...
	\section{Guidelines: Using the Parallel Spatial Locality Metric} \label{guidelines}
	 ...
	 Codes with a mix of favourable and unfavourable access patterns (typically involving two-dimensional matrices such as LUD and SRAD) will have a steep drop-off in PSL to about half of the starting PSL metric as $n$ increases, and the curve flattens out at that point.
	 
	 In the extreme case, when parallel memory accesses are entirely disparate as for Needleman–Wunsch, we observe a flat parallel spatial locality graph.
	 The worst possible memory access pattern: all threads in a work-group accessing results in a flat PSL metric of 1.
